	...
	public class DragDropListBox : ListBox, IDragDropSource
	{
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			if (_dragOriginBox != Rectangle.Empty && !_dragOriginBox.Contains(e.X, e.Y)) { // Initiate drag-and-drop
                ...
                #region Kane's extension below
                Console.WriteLine(this.SelectedIndices.Count + " items selected: ");
                string selectedText = string.Empty; // = listBox1.SelectedItems.ToString();
                foreach (int i in SelectedIndices)
                {
                    //Console.WriteLine(i);
                    selectedText += Items[i] + Environment.NewLine;
                }
                Console.WriteLine(SelectedIndices.Count + " items selected: " + selectedText);
                DoDragDrop(selectedText, DragDropEffects.Copy | DragDropEffects.Move);
                #endregion
